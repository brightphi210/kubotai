    ...
    return (
      <div className="fixed bottom-0 border-t border-neutral-300 w-full text-[10px] bg-white">
        <div className="flex items-center gap-3 justify-between p-5">
          {[
            { name: "Home", icon: <GoHome className="text-2xl" /> },
            { name: "Task", icon: <CiViewList className="text-2xl" /> },
            { name: "Friends", icon: <MdOutlinePeopleAlt className="text-2xl" /> },
            { name: "Leaders", icon: <PiTrophy className="text-2xl" /> },
            { name: "Wallet", icon: <IoWalletOutline className="text-2xl" /> },
          ].map((tab) => (
            <div
              key={tab.name}
